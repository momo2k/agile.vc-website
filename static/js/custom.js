// Override theme toggle - MUST run after Archie's themetoggle.js
(function() {
  // Store original functions if they exist
  var originalSetTheme = window.setTheme;
  var originalToggleTheme = window.toggleTheme;
  
  // Our custom setTheme with SWAPPED icons
  window.setTheme = function(mode) {
    localStorage.setItem("theme-storage", mode);
    var toggleBtn = document.getElementById("dark-mode-toggle");
    
    if (mode === "light") {
      var darkModeStyle = document.getElementById("darkModeStyle");
      if (darkModeStyle) darkModeStyle.disabled = true;
      
      // Light mode → show MOON (to switch TO dark)
      if (toggleBtn) {
        toggleBtn.innerHTML = `<svg class="feather" viewBox="0 0 24 24" fill="none" stroke="#717074" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>
        </svg>`;
        toggleBtn.setAttribute('data-theme', 'light');
        toggleBtn.blur();
      }
    } else if (mode === "dark") {
      var darkModeStyle = document.getElementById("darkModeStyle");
      if (darkModeStyle) darkModeStyle.disabled = false;
      
      // Dark mode → show SUN (to switch TO light)
      if (toggleBtn) {
        toggleBtn.innerHTML = `<svg class="feather" viewBox="0 0 24 24" fill="none" stroke="#f5f5f5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="12" cy="12" r="5"></circle>
          <line x1="12" y1="1" x2="12" y2="3"></line>
          <line x1="12" y1="21" x2="12" y2="23"></line>
          <line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line>
          <line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line>
          <line x1="1" y1="12" x2="3" y2="12"></line>
          <line x1="21" y1="12" x2="23" y2="12"></line>
          <line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line>
          <line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line>
        </svg>`;
        toggleBtn.setAttribute('data-theme', 'dark');
        toggleBtn.blur();
      }
    }
  };
  
  window.toggleTheme = function() {
    if (localStorage.getItem("theme-storage") === "light") {
      window.setTheme("dark");
    } else if (localStorage.getItem("theme-storage") === "dark") {
      window.setTheme("light");
    }
  };
  
  // Re-initialize on load to fix initial state
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      var savedTheme = localStorage.getItem("theme-storage") || "light";
      window.setTheme(savedTheme);
    });
  } else {
    // DOM already loaded
    var savedTheme = localStorage.getItem("theme-storage") || "light";
    window.setTheme(savedTheme);
  }
})();

// Prevent header link from reloading page on homepage
document.addEventListener('DOMContentLoaded', function() {
  var headerLink = document.querySelector('header .main a');
  if (headerLink && window.location.pathname === '/') {
    headerLink.addEventListener('click', function(e) {
      e.preventDefault();
      this.blur();
    });
  }
});
